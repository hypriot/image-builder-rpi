...
describe command('dpkg -l docker-ce') do
  ...
  its(:stdout) { should match /17.09.0~ce~rc3-0~r/ }
...
describe command('docker -v') do
  its(:stdout) { should match /Docker version 17.09.0-ce-rc3, build/ }
  its(:exit_status) { should eq 0 }
end

describe command('docker version') do
  its(:stdout) { should match /Client:. Version:      17.09.0-ce-rc3. API version:  1.32/m }
  its(:stdout) { should match /Server:. Version:      17.09.0-ce-rc3. API version:  1.32/m }
  its(:exit_status) { should eq 0 }
end
