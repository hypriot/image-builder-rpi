...
describe command('dpkg -l docker-ce') do
  ...
  its(:stdout) { should match /5:19.03.6~2.2.rc2-0~raspbian/ }
...
describe command('docker -v') do
  its(:stdout) { should match /Docker version 19.03.6-rc2, build/ }
  its(:exit_status) { should eq 0 }
end

describe command('docker version') do
  its(:stdout) { should match /Client: Docker Engine - Community. Version:           19.03.6-rc2. API version:       1.40/m }
  its(:stdout) { should match /Server: Docker Engine - Community. Engine:.  Version:          19.03.6-rc2.  API version:      1.40/m }
  its(:exit_status) { should eq 0 }
end
